...
void DrawHeader_Ani(png_texture icon, char * headerTitle, char * headerSubTitle, u32 rgba, u32 bgrgba, int ani, int div)
{
	...
	if (headerSubTitle)
	{
		int width = 800 - (MENU_ICON_OFF + MENU_TITLE_OFF + WidthFromUTF8(headerTitle)) - 30;
		SetFontSize(20, 20);
		char * tName = malloc(strlen(headerSubTitle) + 1);
		strcpy(tName, headerSubTitle);
		while (WidthFromUTF8(tName) > width)
		{
...
void loading_screen_thread(void* user_data)
{
    ...
    while (please_wait == 1)
    {
		...
		DrawString(0, 336, (char*) user_data);		
...
int init_loading_screen(const char* message)
{
	...
	SetFontAlign(1);
	SetFontSize(14, 14);
	SetFontColor(0x00000000 | 0xFF, 0);
